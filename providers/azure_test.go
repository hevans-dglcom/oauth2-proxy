...
func TestAzureProviderEnrichSession(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			ExpectedError:           fmt.Errorf("unable to get email address from profile URL: %v", errors.New("type assertion to string failed")),
		},
		{
...
func TestAzureProviderProtectedResourceConfiguredOAuthV2(t *testing.T) {
	...
	p.isV2Endpoint = true
	result := p.GetLoginURL("https://my.test.app/oauth", "", "", url.Values{})
	assert.Contains(t, result, url.QueryEscape("http://my.resource.test"))
	assert.NotContains(t, result, "resource="+url.QueryEscape("http://my.resource.test"))
}
